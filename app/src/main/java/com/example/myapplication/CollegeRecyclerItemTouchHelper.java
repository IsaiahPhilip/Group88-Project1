...
/**
 * Allows for swiping between for CollegeClasses so they can be edited/deleted
 */
public class CollegeRecyclerItemTouchHelper extends ItemTouchHelper.SimpleCallback {
    private CollegeClassAdapter adapter;
    

    /**
     * Constructor that fills adapter with a new CollegeClassAdapter
     * @param adapter the adapter to be set
     */
    public CollegeRecyclerItemTouchHelper(CollegeClassAdapter adapter) {
        super(0, ItemTouchHelper.LEFT | ItemTouchHelper.RIGHT); //allows for left and right swiping
        this.adapter = adapter;
    }

    /**
     * We don't use this method, just needs to be defined so the interface can be implemented
    ...
    public void onSwiped(final RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        if (direction == ItemTouchHelper.RIGHT) {
            ...
            builder.setTitle("Delete Class"); //sets title of alert
            builder.setMessage("Do you want to delete this class?"); //sets message of alert
            builder.setPositiveButton("Yes", new DialogInterface.OnClickListener() { //creates a button for confirming deletion
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    adapter.deleteClassItem(position); //if button is clicked, class will be deleted.
                }
            });
            ...
            adapter.editClassItem(position); //calls the edit method already defined.
        }
    }
