...
/**
 * Cpmplex method, that said it is relatively straightforward. This is what allows
 * our swiping to work and delete/edit tasks
 */
public class AssignmentRecycleTouchHelper extends ItemTouchHelper.SimpleCallback {
    private AssignmentAdapter adapter;

    /**
     * Constructor for the touchHelper
     * @param adapter the adapter that will be set
     */

    public AssignmentRecycleTouchHelper(AssignmentAdapter adapter) {
    ...
    public void onSwiped(final RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        if (direction == ItemTouchHelper.RIGHT) {
            ...
            builder.setTitle("Delete task"); //sets title of alert
            builder.setMessage("Do you want to delete this task?"); //sets message of alert
            builder.setPositiveButton("Yes", new DialogInterface.OnClickListener() { //creates a button for confirming deletion
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    adapter.deleteItem(position); //if button is clicked, task will be deleted.
                }
            });
            ...
            adapter.editItem(position); //calls the edit method already defined.
        }
    }
